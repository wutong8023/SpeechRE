...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # breakpoint()
        rel_precision, rel_recall, rel_f1, total = self.compute_re(net_output, sample)
        sample_size = (
    ...
    def compute_re(self, net_output, sample):
        ...
        for i in range(target.shape[0]):
            target_triplets = self.extract_triplets(target[i].tolist())
            output_triplets = self.extract_triplets(output[i].tolist())
            target_relations = []
            output_relations = []
